...
def load_models(args):
    ...
    clip_pretrained = args.options.get(
        'clip_pretrained',
        os.environ.get("GENEVAL_CLIP_PRETRAINED", "laion2b_s32b_b82k"),
    )
...
if __name__ == "__main__":
    ...
    full_results = []
    subfolders = sorted(
        [f for f in os.listdir(args.imagedir) if os.path.isdir(os.path.join(args.imagedir, f)) and f.isdigit()],
        key=int,
    )
