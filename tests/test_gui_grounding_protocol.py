...
class Mind2WebProtocolTest(unittest.TestCase):
    def test_builds_paper_style_target_explicit_click_prompt(self):
    ...
    def test_type_and_select_keep_their_values(self):
    ...
    def test_structured_operation_is_authoritative(self):
        target = parse_target_action(
    ...
    def test_task_history_is_an_explicitly_separate_protocol(self):
        target = parse_target_action(
            "[button] Submit -> CLICK",
            {"op": "CLICK"},
        )
    ...
    def test_missing_description_uses_candidate_fallback(self):
        target = parse_target_action(
            "-> CLICK",
            {"op": "CLICK"},
    ...
    def test_crop_plan_is_balanced_deterministic_and_unique(self):
        source_ids = ["a", "b", "c"]
        plan = mind2web_crop_plan(source_ids, target_count=8, seed=42)

        self.assertEqual(
            plan,
            mind2web_crop_plan(source_ids, target_count=8, seed=42),
        )
    ...
    def test_crop_plan_rejects_invalid_inputs(self):
        with self.assertRaises(ValueError):
            mind2web_crop_plan([], target_count=1, seed=42)
        with self.assertRaises(ValueError):
            mind2web_crop_plan(["duplicate", "duplicate"], target_count=1, seed=42)

    def test_paired_protocols_receive_same_order_independent_seed(self):
